...
export function HomeView({ onSelect, macro, onMacroChange, onResetMacro }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pt-8 sm:pt-12 pb-16 sm:pb-20 fade-in">
      {/* Hero */}
      <div className="text-center mb-8 sm:mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-300 text-[11px] sm:text-xs mb-4 sm:mb-6">
          <Sparkles className="w-3.5 h-3.5" />
          <span>5인의 거장이 자신의 철학으로 진단합니다</span>
        </div>
        <h1 className="text-[1.65rem] leading-[1.18] sm:text-4xl md:text-5xl font-bold tracking-tight mb-3 sm:mb-4 px-2">
          미국 주식,{' '}
          <span className="text-amber-300 whitespace-nowrap">거장의 눈</span>
          으로<br className="sm:hidden" /> 본다면.
        </h1>
        <p className="text-sm sm:text-base text-slate-400 max-w-2xl mx-auto leading-relaxed px-2">
      ...
      <div className="max-w-2xl mx-auto mb-6 sm:mb-8">
        <SearchBar onSelect={onSelect} autoFocus={false} />
      </div>
      ...
      <div className="max-w-2xl mx-auto space-y-3 text-sm">
        {recentTickers.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-slate-500 mr-1">최근:</span>
            {recentTickers.map((t) => (
              <button
                key={t.ticker}
                onClick={() => onSelect(t)}
                className="chip hover:bg-slate-800 hover:border-slate-600"
              >
                <span className="font-mono text-amber-300">{t.ticker}</span>
                <span className="text-slate-500 hidden xs:inline">{t.nameKo}</span>
              </button>
            ))}
          </div>
        )}
      ...
      {/* 5인 소개 — 모바일에선 가로 스냅 스크롤 */}
      <div className="mt-12 sm:mt-16">
        <h2 className="text-center text-[10px] sm:text-xs uppercase tracking-[0.3em] text-slate-500 mb-4 sm:mb-6">The Five Legends</h2>

        {/* 모바일: 가로 스크롤 */}
        <div className="sm:hidden -mx-4 px-4">
          <div className="scroll-x flex gap-3 pb-3 snap-x">
            {PERSONA_ORDER.map((id) => {
              const p = PERSONAS[id];
              return (
                <div
                  key={id}
                  className="card p-4 shrink-0 w-[60vw] min-w-[200px] max-w-[260px] snap-start"
                >
        ...
        <div className="hidden sm:grid grid-cols-2 lg:grid-cols-5 gap-3">
          {PERSONA_ORDER.map((id) => {
            ...
            return (
              <div
                key={id}
                className="card p-4 hover:-translate-y-0.5 hover:border-slate-600"
              >
                <div className={`text-2xl mb-2 ${p.themeColor}`}>{p.symbol}</div>
                <div className="text-sm font-semibold">{p.nameKo}</div>
                <div className="text-[10px] text-slate-500 mb-2">{p.nameEn}</div>
